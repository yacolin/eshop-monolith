---
name: "commit-message"
description: "生成符合项目规范的git commit message。当用户需要提交代码、生成commit message、询问commit格式时调用。适用于用户说'commit'、'提交'、'生成commit'、'写提交信息'等场景。生成的格式为<type>(<scope>): <description>，使用中文描述。"
---
...
- 详细描述更改的原因和影响
- 使用无序列表（`-` 开头），每条独立一行，**不留空行**
- 每行不超过72个字符
- 关联 issue 直接作为一条列表项，不加额外空行

### 页脚 (Footer) [可选]

- 在无序列表末尾添加，与正文列表紧接，不留空行
- 破坏性更改的警告
- 其他重要信息

### 禁止项

- **不要**在 commit message 中添加 `Co-Authored-By: Claude <noreply@anthropic.com>` 行
