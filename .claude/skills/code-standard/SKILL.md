...
- **cmd目录**: 存放各服务的入口文件
- **internal目录**: 按业务模块划分
  - **{module}/api**: 包含HTTP处理、路由和DTO
  - **{module}/domain**: 包含领域模型和仓储接口
  - **{module}/service**: 包含业务逻辑
- **internal/eventbus**: 内部事件总线，替代消息队列实现模块间解耦
...
1. **代码复用**: 提取公共代码到pkg目录
2. **接口设计**: 使用接口定义仓储和服务契约
3. **事件驱动**: 使用 eventbus 实现模块间解耦
4. **配置管理**: 使用viper管理配置
5. **日志管理**: 使用zap进行结构化日志记录
6. **事务管理**: 跨模块操作使用本地事务保证ACID
7. **幂等性**: 关键接口使用Redis实现幂等性控制
8. **CI/CD**: 配置持续集成和持续部署
