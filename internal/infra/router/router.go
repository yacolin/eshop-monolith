...
func SetupRouter(cfg *config.Config, repos *repository.Repositories, db *gorm.DB, mqClient *rabbitmq.Client) *gin.Engine {
	...
	v1 := router.Group("/api/v1")
	{
		...
		trade.RegisterTradeRoutes(v1, db, mqClient, repos.Redis, func() { dashboardSvc.InvalidateCache(context.Background()) })
		...
		staff.RegisterStaffRoutes(v1, db)
		staff.RegisterRoleRoutes(v1, db)
		staff.RegisterPermissionRoutes(v1, db)
